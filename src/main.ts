...
function scheduleViewportSync(): void {
  game.forceViewportSync();
  window.setTimeout(() => game.forceViewportSync(), 80);
  window.setTimeout(() => game.forceViewportSync(), 200);
  window.setTimeout(() => game.forceViewportSync(), 450);
}
...
const screenOrientation = (screen as Screen & { orientation?: ScreenOrientation }).orientation;
if (screenOrientation && typeof screenOrientation.addEventListener === "function") {
  screenOrientation.addEventListener("change", () => scheduleViewportSync());
}
if (window.visualViewport) {
  window.visualViewport.addEventListener("resize", () => game.forceViewportSync());
}
