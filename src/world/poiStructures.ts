...
export function createPoiStructure(poi:GeneratedPoi):StructureCollisionDefinition{
 ...
 const surface=(id:string,kind:StructureSurfaceRecord["kind"],size:readonly[number,number,number],position:readonly[number,number,number])=>surfaces.push({id:`${poi.id}:${id}`,kind,centre:world(poi,...position),length:size[0],width:size[2],direction:d,startHeight:poi.position.y+position[1]+size[1]/2,endHeight:poi.position.y+position[1]+size[1]/2,crownHeight:0,thickness:size[1],overhead:true});
