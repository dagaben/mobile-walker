...
import { BRIDGE_ARCHETYPES, generateBridges, scoreBridgeArchetypes, spanSuitability, type BridgeArchetype, type BridgeCrossingCandidate } from "./bridges";
import { isRiverAt } from "./terrainSampling";
import { pointInFootprint } from "./poi";

function context(biome:BridgeCrossingCandidate["biome"]):BridgeCrossingCandidate {
 return {id:`test:${biome}`,ownerChunk:{x:0,z:0},centre:{x:8,z:8},riverTangent:{x:0,z:1},crossingDirection:{x:1,z:0},leftBankAnchor:{x:4,y:.2,z:8},rightBankAnchor:{x:12,y:.25,z:8},approachPoints:{left:{x:1,y:.2,z:8},right:{x:15,y:.25,z:8}},spanLength:8,approachSlope:.02,bankStability:.9,biome,accepted:false};
}
...
describe("deterministic span POIs",()=>{
 ...
 it("is deterministic, perpendicular, bank-to-bank, excluded, and singly owned",()=>{
  ...
  expect(isRiverAt(Number.parseInt(bridge.id.split(":")[1]!,16),bridge.leftBankAnchor.x,bridge.leftBankAnchor.z)).toBe(false);
  expect(isRiverAt(Number.parseInt(bridge.id.split(":")[1]!,16),bridge.rightBankAnchor.x,bridge.rightBankAnchor.z)).toBe(false);
 ...
 it("does not allocate candidate debug records in normal chunk output",async()=>{
  ...
  expect(generateChunk(7,{x:0,z:0},undefined,true).bridgeCandidates).toHaveLength(1);
 });
});
