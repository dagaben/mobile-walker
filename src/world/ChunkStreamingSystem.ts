...
export interface ChunkStreamingDiagnostics {
  generationQueued: number; generationInProgress: number; awaitingActivation: number; partiallyActivated: number;
  lastWorkerGenerationMs: number; maxWorkerGenerationMs: number; lastTransferDelayMs: number; maxTransferDelayMs: number;
...
interface WorkerReply { requestId: number; data: GeneratedChunkData; generationMs: number; finishedAt: number; }
function createWorkerGenerator(onTiming: (generation: number, transfer: number) => void, now: () => number): { generate: ChunkGenerator; dispose: () => void } | undefined {
  ...
  worker.onmessage = (event: MessageEvent<WorkerReply>) => {
    ...
    onTiming(event.data.generationMs, Math.max(0, now() - event.data.finishedAt));
  ...
  return { generate: (seed, coordinate) => new Promise((resolve, reject) => { const requestId = nextRequestId++; pending.set(requestId, { resolve, reject }); worker.postMessage({ requestId, seed, coordinate }); }), dispose: () => { worker.terminate(); for (const p of pending.values()) p.reject(new Error("Chunk streaming disposed")); pending.clear(); } };
}
...
export class ChunkStreamingSystem implements RenderSystem {
  ...
  private readonly cacheSize: number; private readonly prefetchRings: number; private readonly now: () => number;
  ...
  private diagnostics: ChunkStreamingDiagnostics = { generationQueued: 0, generationInProgress: 0, awaitingActivation: 0, partiallyActivated: 0, lastWorkerGenerationMs: 0, maxWorkerGenerationMs: 0, lastTransferDelayMs: 0, maxTransferDelayMs: 0, activationMsThisFrame: 0, longestActivationStage: "none", longestActivationStageMs: 0, cacheHits: 0, cacheMisses: 0, firstRenderMs: 0, stageTimes: {} };
  ...
  constructor(private readonly scene: THREE.Scene, private readonly seed: number | string, private readonly radius = 1, options: ChunkStreamingOptions = {}) {
    ...
    this.prefetchRings = Math.max(0, options.prefetchRings ?? (options.generator ? 0 : 1)); this.offsets = resolveNeighborhoodOffsets(this.radius, options.offsets);
  }
  ...
  private selectNeighborhood(): void {
    ...
    const ring = this.prefetchRings;
    for (let z = this.center.z - this.offsets.north - ring; z <= this.center.z + this.offsets.south + ring; z++) for (let x = this.center.x - this.offsets.west - ring; x <= this.center.x + this.offsets.east + ring; x++) {
  ...
  private processGeneration(): void {
    for (const [id, coordinate] of this.ordered(this.requestedAdditions).slice(0, this.generationWorkPerFrame)) {
      ...
      try { const result=this.generator(this.seed,coordinate); if(result instanceof Promise) void result.then(data=>this.finishGeneration(id,data),()=>this.retryGeneration(id,coordinate)); else { this.recordWorkerTiming(this.now()-start,0); this.finishGeneration(id,result); } } catch { this.retryGeneration(id,coordinate); }
    }
  }
  private finishGeneration(id: ChunkId,data: GeneratedChunkData): void { this.generating.delete(id); if(this.disposed)return; if(!this.dataWanted.has(id)) this.putInCache(id,{data}); else this.ready.set(id,{data}); }
  ...
  private processMeshes(): void {
    ...
    for(const [id,job] of this.ordered(this.jobs)) {
      while(!job.complete) {
        ...
        const timing=job.step(); if(!timing) break; this.recordStage(timing.stage,timing.milliseconds);
  ...
  private activateComplete(id:ChunkId,data:GeneratedChunkData,group:THREE.Group):void { this.meshes.registerGroup(group); this.active.set(id,group);this.activeData.set(id,data);this.scene.add(group); }
  ...
  private putInCache(id:ChunkId,r:CachedChunk):void { if(this.cacheSize===0||this.disposed){if(r.group)this.meshes.disposeChunk(r.group);return;} const old=this.cache.get(id);if(old?.group&&old.group!==r.group)this.meshes.disposeChunk(old.group);this.cache.delete(id);this.cache.set(id,r);while(this.cache.size>this.cacheSize){const key=this.cache.keys().next().value as ChunkId;const value=this.cache.get(key);this.cache.delete(key);if(value?.group)this.meshes.disposeChunk(value.group);} }
  private recordWorkerTiming(g:number,t:number):void { this.diagnostics.lastWorkerGenerationMs=g;this.diagnostics.maxWorkerGenerationMs=Math.max(this.diagnostics.maxWorkerGenerationMs,g);this.diagnostics.lastTransferDelayMs=t;this.diagnostics.maxTransferDelayMs=Math.max(this.diagnostics.maxTransferDelayMs,t); }
  ...
  private updateDiagnosticCounts():void { this.diagnostics.generationQueued=this.requestedAdditions.size;this.diagnostics.generationInProgress=this.generating.size;this.diagnostics.awaitingActivation=[...this.ready.keys()].filter(id=>this.wanted.has(id)).length;this.diagnostics.partiallyActivated=this.jobs.size; }
