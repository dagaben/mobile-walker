...
registerPoiDefinition({id:"plains-farmhouse",label:"Plains farmhouse",biomes:{allowed:["plains"],preferred:["plains"]},...PLAINS_FARMHOUSE_CONFIG,weight:1,footprint:{kind:"rectangle",width:7,depth:6},shadowCaster:{width:6.4,depth:5.2,height:2.7},hydrology:{rejectRiverIntersection:true},terrain:{maximumSlope:.13,maximumVariation:.72},renderer:"plains-farmhouse",debugColor:0xe7bd72,suitability:c=>c.plainsCoverage<.72?{score:0,reason:"insufficient plains footprint"}:{score:.5+c.plainsCoverage*.35-c.terrain.approximateSlope,reason:c.distanceToLake<12?"lake requirement not met":undefined},parameters:(seed,x,z)=>({roofHue:hashFloat(seed,x,z,923)})});
...
export function isDryPoiFootprint(seedInput:number|string,shape:PoiFootprint):boolean{const seed=normalizeSeed(seedInput);return !footprintIntersectsRiver(seed,shape)&&footprintLakeCoverage(seed,shape)===0&&footprintAvoidsWetlandPools(seed,shape);}
...
function evaluate(seed:number,d:PoiDefinition,cellX:number,cellZ:number,index=0):Evaluated{
  ...
  if(!d.hydrology?.shoreline&&(isLakeAt(seed,x,z)||(d.hydrology?.rejectRiverIntersection&&isRiverAt(seed,x,z))))return boundedSet(candidateCache,cacheKey,{definition:d,id,cellX,cellZ,index,x,z,rotation,footprint,context:defaultContext(seed,x,z,rotation,footprint,coarseBiome),score:0,reason:isLakeAt(seed,x,z)?"underwater":"river intersection"},MAX_CANDIDATE_CACHE);
  ...
  const clearingIsDry=d.id!=="forest-cabin"||biome.dominant!=="wetland"||isDryPoiFootprint(seed,{kind:"circle",x,z,radius:d.clearanceRadius});
  const context:PoiSuitabilityContext={seed,x,z,rotation,biome,terrain,footprint,distanceToRiver,distanceToLake,intersectsRiver,underwater,vegetationDensity,plainsCoverage,localProminence,clearingIsDry,shoreline};
  let reason:PoiRejectionReason|undefined;
  if(underwater||footprintLakeCoverage(seed,footprint)>0)reason="underwater";else if(d.hydrology?.rejectRiverIntersection&&intersectsRiver)reason="river intersection";else if(d.hydrology?.requireLakeWithin!==undefined&&distanceToLake>d.hydrology.requireLakeWithin)reason="lake requirement not met";else if(terrain.approximateSlope>d.terrain.maximumSlope)reason="slope too high";else if(terrain.heightVariation>d.terrain.maximumVariation)reason="uneven terrain";
...
export function generatePois(seedInput:number|string,coordinate:ChunkCoordinate):Readonly<{pois:readonly GeneratedPoi[];candidates:readonly PoiDebugCandidate[]}>{
  ...
  for(const definition of definitions.values()){const margin=definition.id==="highland-watchtower"?Math.ceil((definition.spacingByType?.[definition.id]??definition.minimumSpacing)/CELL_SIZE)+1:ORDINARY_SEARCH_MARGIN;for(let cz=baseZ-margin;cz<=endZ+margin;cz++)for(let cx=baseX-margin;cx<=endX+margin;cx++)all.push(evaluate(seed,definition,cx,cz));}
